...
var solarSshApp = function(sshUrlHelper, options) {
  ...
  var termSettings = new SshTerminalSettings(
    config.cols || 100,
    config.lines || 24
  );
  ...
  function createSession() {
    ...
    return executeWithPreSignedAuthorization("GET", url, auth)
      ...
      .on("error", function(xhr) {
        ...
        termWriteFailed();
        termWriteBrightRed(
          "Failed to get request new SSH session: " + xhr.responseText,
          true
        );
      });
  }
    ...
    function executeQuery() {
      ...
      req
        ...
        .on("error", function(xhr) {
          ...
          termWriteFailed();
          termWriteBrightRed(
            "Failed to get SolarNode remote SSH session start status: " +
              xhr.responseText,
            true
          );
        })
  ...
  function webSocketOpen() {
    ...
    console.log(
      "Authenticating web socket connection with message %s",
      msg.toJsonEncoding()
    );
  ...
  function webSocketClose(event) {
    console.log(
      "ws close event: code = %d; reason = %s",
      event.code,
      event.reason
    );
    resetWebSocket();
  ...
  function webSocketMessage(event) {
    ...
    if (msg.success) {
      ...
      termWriteFailed();
      termWriteBrightRed(
        "Failed to attach to SSH session: " + event.data,
        true
      );
      socket.close();
    }
